...
+(function ($) {
  ...
  function getTargetFromTrigger($trigger) {
    let href;
    const target = $trigger.attr('data-target') || ((href = $trigger.attr('href')) && href.replace(/.*(?=#[^\s]+$)/, '')); // strip for ie7
...
+(function ($) {
  ...
  Tooltip.prototype.enter = function (obj) {
    let self = obj instanceof this.constructor ? obj : $(obj.currentTarget).data(`bs.${this.type}`);
