...
+(function (a) {
  ...
  a(document)
    ...
    .on('focus.bs.button.data-api blur.bs.button.data-api', '[data-toggle^="button"]', (b) => {
      a(b.target)
        .closest('.btn')
        .toggleClass('focus', b.type == 'focus');
    });
}(jQuery)),
+(function (a) {
  function b(b) {
  ...
  (c.prototype.getItemForDirection = function (a, b) {
    const c = a == 'prev' ? -1 : 1;
    const d = this.getItemIndex(b);
    const e = (d + c) % this.$items.length;
  ...
  (c.prototype.slide = function (b, d) {
    ...
    let f = d || this.getItemForDirection(b, e);
    const g = this.interval;
    const h = b == 'next' ? 'left' : 'right';
    const i = b == 'next' ? 'first' : 'last';
    const j = this;
    if (!f.length) {
    ...
    const k = f[0];
    const l = a.Event('slide.bs.carousel', { relatedTarget: k, direction: h });
    if ((this.$element.trigger(l), !l.isDefaultPrevented())) {
      if (((this.sliding = !0), g && this.pause(), this.$indicators.length)) {
        this.$indicators.find('.active').removeClass('active');
        const m = a(this.$indicators.children()[this.getItemIndex(f)]);
        m && m.addClass('active');
      }
      const n = a.Event('slid.bs.carousel', { relatedTarget: k, direction: h });
      return (
...
+(function (a) {
  function b(b) {
    let c;
    const d = b.attr('data-target') || ((c = b.attr('href')) && c.replace(/.*(?=#[^\s]+$)/, ''));
    return a(d);
  }
  function c(b) {
    return this.each(function () {
  ...
  (d.prototype.show = function () {
    if (!this.transitioning && !this.$element.hasClass('in')) {
      let b;
      const e = this.$parent && this.$parent.find('> .panel').children('.in, .collapsing');
  ...
  a(document).on('click.bs.collapse.data-api', '[data-toggle="collapse"]', function (d) {
    ...
    const h = g ? 'toggle' : a.extend({}, e.data(), { trigger: this });
    c.call(f, h);
  });
}(jQuery)),
+(function (a) {
  ...
  (g.prototype.keydown = function (b) {
    if (/(38|40|27|32)/.test(b.which)) {
      ...
      if ((b.preventDefault(), b.stopPropagation(), !d.is('.disabled, :disabled'))) {
        ...
        if ((!g && b.which != 27) || (g && b.which == 27)) return b.which == 27 && e.find(f).trigger('focus'), d.trigger('click');
        const h = ' li:not(.divider):visible a';
        const i = e.find(`[role="menu"]${h}, [role="listbox"]${h}`);
        if (i.length) {
          let j = i.index(b.target);
          b.which == 38 && j > 0 && j--,
          b.which == 40 && j < i.length - 1 && j++,
          ~j || (j = 0),
...
+(function (a) {
  ...
  a(document).on('click.bs.modal.data-api', '[data-toggle="modal"]', function (c) {
    ...
    const g = f.data('bs.modal') ? 'toggle' : a.extend({ remote: !/#/.test(e) && e }, f.data(), d.data());
...
+(function (a) {
  ...
  (c.prototype.show = function () {
    ...
    if (this.hasContent() && this.enabled) {
      ...
      if (j) {
        const n = h;
        const o = this.options.container ? a(this.options.container) : this.$element.parent();
        const p = this.getPosition(o);
        (h = h == 'bottom' && k.bottom + m > p.bottom
          ? 'top'
          : h == 'top' && k.top - m < p.top
            ? 'bottom'
            : h == 'right' && k.right + l > p.width
              ? 'left'
              : h == 'left' && k.left - l < p.left
                ? 'right'
                : h),
        f.removeClass(n).addClass(h);
      }
      const q = this.getCalculatedOffset(h, k, l, m);
  ...
  (c.prototype.getPosition = function (b) {
    ...
    e.width == null && (e = a.extend({}, e, { width: e.right - e.left, height: e.bottom - e.top }));
    const f = d ? { top: 0, left: 0 } : b.offset();
    const g = { scroll: d ? document.documentElement.scrollTop || document.body.scrollTop : b.scrollTop() };
    const h = d ? { width: a(window).width(), height: a(window).height() } : null;
...
+(function (a) {
  ...
  (c.prototype.getState = function (a, b, c, d) {
    ...
    if (this.affixed == 'bottom') return c != null ? (e + this.unpin <= f.top ? !1 : 'bottom') : a - d >= e + g ? !1 : 'bottom';
    const h = this.affixed == null;
    const i = h ? e : f.top;
    const j = h ? g : b;
    return c != null && c >= i ? 'top' : d != null && i + j >= a - d ? 'bottom' : !1;
  }),
