        ...
        <el-form ref="form" :model="form" :rules="rules">
          ...
          <el-form-item label="活动有效期" prop="" class="required">
            ...
            <el-date-picker
              ...
              :picker-options="pickerOptions"
            ></el-date-picker>
            <el-checkbox v-model="form.dateSign" :true-label="2" :false-label="1" :disabled="this.form.giftType == 3">
              不限结束时间
            </el-checkbox>
          </el-form-item>
          ...
          <el-form-item label="礼品类型" class="required">
            <el-radio-group v-model="form.giftType" @change="gifttypeChange">
              <el-radio :disabled="true" class="radio" :label="1">海贝积分</el-radio>
              <el-radio :disabled="true" class="radio" :label="2">卡萨帝积分</el-radio>
              <el-radio :disabled="true" class="radio" :label="3">实物礼品</el-radio>
              <el-radio :disabled="true" v-if="form.rightSmallType === 1" class="radio" :label="5">虚拟礼品 </el-radio>
          ...
          <el-form-item label="投入预算" prop="budgetAmount">
            ...
            ></el-input>
            <span>元</span>
            <span style="color:#ff4949;font-size:12px;" v-if="integralLoading"
              >积分中心剩余预算 {{ integralBalance }}（元）</span
            >
            <!--<span v-if="!remainAmountLoading" style="color:#ff4949;font-size:12px;">海贝商城账户剩余权益金额：{{remainAmount.remainHaier}}元，卡萨帝权益商城余额：{{remainAmount.remainCasarte}}元</span>-->
            <!--<span v-if="remainAmountLoading" style="color:#ff4949;font-size:12px;">海贝商城账户剩余权益金额：暂未获取，卡萨帝权益商城余额：暂未获取</span>-->
          </el-form-item>
          ...
          <el-form-item>
            <el-button type="primary" class="trs-btn" style="width:109px;margin-top:14px;" @click="next"
              >下一步</el-button
            >
            <!-- <el-button type="primary" class="trs-btn" style="width:150px;margin-top:14px;" @click="back">完成并返回列表</el-button> -->
          </el-form-item>
        </el-form>
      </div>
    </div>
    ...
    back(temporary) {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          axios
            .post(`${this.apiUrl}vipcenter/orderRights/modifyOrderRights`, {
              auditPics: data.auditPics,
              budgetAmount: this.form.budgetAmount,
              channelType: this.form.channelType,
              crTime: this.form.crTime,
              ...
              id: this.form.id,
              jfAmount: this.integralBalance,
              recognitionFlag: this.form.recognitionFlag,
            ...
            .then((response) => {
              ...
              if (response.data.isSuccess == true) {
                ...
                _this.$router.push('/rights/rightsactlist');
              } else {
                _this.$message({
          ...
          axios
            ...
            .then((data) => {
              if (data) {
                ...
                this.$router.push({ path: '/rights/rightsRangetwo', query: { id: _this.form.id } });
              }
            });
        }
      });
