      ...
      <div class="config-form">
        ...
        <el-form ref="form" :model="form" :rules="rules">
          ...
          <el-form-item label="是否认筹" class="required">
            ...
            <span
              v-show="form.recognitionFlag == 1"
              style="color:rgb(255, 73, 73);font-size:12px;position: relative;bottom: 3px;"
              >（认筹时间与活动有效期一致）</span
            >
          </el-form-item>
  ...
  methods: {
    gifttypeChange(e) {
      ...
      this.requestRights.getRemainMoney(data).then((data) => {
        ...
        that.$refs.form.validateField('budgetAmount', () => {});
      });
      if (e == 1) {
    ...
    getOrderRightsById(id) {
      ...
      this.requestRights.getRightsById(params).then((data) => {
        if (data) {
          ...
          _this.form.startTime = data.startTime; // 权益开始时间
          _this.form.exchangeStartTime = data.exchangeStartTime; // 权益兑换开始时间
          ...
          if (data.endTime.indexOf('2099') == 0) {
            ...
            _this.form.exchangeEndTime = data.exchangeEndTime; // 权益兑换结束时间
            _this.form.endTime = data.endTime; // 权益结束时间
          }
        }
      });
    }
