  ...
  >
    <div class="rights-config-header-box">
      ...
      <div class="save-draft">
        <span>有急事？先</span>
        <a href="javascript:;" @click="save({temporary: true})">存草稿</a>
        <span>回来继续配置</span>
      </div>
    </div>
            ...
            <div class="gift-list" style="width:700px;">
              <div v-if="radio===1">
                ...
                <p style="margin-bottom: 15px;">购买限制
                  <el-select v-model="form.coupleCategoryNumber" placeholder="请选择"
              ...
              <div v-if="radio==2" style="margin-top:18px;">
                <p style="margin-bottom: 15px;">购买限制
                  <el-input v-if="radio==2" :maxlength="2" v-model="form.coupleNumber" placeholder="请输入"
                            style="width: 80px;" @input.native="couChange"></el-input>
                ...
                <el-tabs v-model="type4TabShow" type="card" @tab-click="handleClick">
                  ...
                  <el-tab-pane label="产业内所有产品参加活动" name="1">
                    <div class="type4-tab-panel">
                      购买 <span class="color-red">{{productCategoryName}}</span>, 满
                      <el-input style="width: 71px;margin:0 10px;" :maxlength="9"
                                v-model="buyAmount"
                  ...
                  <el-tab-pane label="产业特定产品参加活动" name="2">
                    <div class="type4-tab-panel">
                      ...
                      <div class="gift-item" v-for="(right, index) in type6Config.rightList" :key="index">
                        <p>满{{right.buyAmount}}-{{right.maxBuyAmount}}元
                          <a v-if="!right.scoreList  && !right.giftList " href="javascript:;"
              ...
              <div v-if="radio==3">
                <div class="select-product-list">
                  <div class="select-product" v-for="(product, index) in productList">
                    ...
                    <el-autocomplete style="width: 435px;" placeholder="输入参加活动的产品型号"
              ...
              <div class="lint-product-list">
                <div class="lint-product" v-for="(product, index) in lintList"
                  ...
                  <el-autocomplete style="width: 250px;" v-model="product.desc"
                                   ...
                                   placeholder="产品型号可为空，默认所有型号" value-key="desc"
            ...
            <div class="lint-everybox" v-if="newlimit.indexOf(2)!=-1">
              ...
              <div>
                ...
                <el-autocomplete style="width: 250px;" v-model="productModel2"
                                 ...
                                 placeholder="产品型号必填" value-key="desc"
    ...
    <el-dialog
      title="导入型号"
      size="tiny"
      :visible.sync="isImportExcelDialog"
      class="gift-rights-dialog trs-dialog"
      :modal-append-to-body="false"
    >
      <div class="rightsgift-importExcel">
        <p class="rightsgift-importExcel-tips">
          <span class="rightsgift-importExcel-tips-cnt">请选择导入文件（xls格式）</span>
          <a :href="xlsDownloadUrl" class="rightsgift-importExcel-tips-href">下载模板</a>
        </p>
        <div v-if="isImportExcelDialog">
          <label for="chooseExcel" class="rightsgift-importExcel-choose">选择文件</label>
          <input
            type="file"
            id="chooseExcel"
            @change="resolveExcel"
          >
          <span>{{choosedExcelName}}</span>
        </div>
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button @click="isImportExcelDialog = false">取 消</el-button>
        <el-button type="primary" @click="pushDataToSelectProduct">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
        ...
        if (this.radio === 2) {
          ...
          for (let i = 0; i < this.type3Config.rightList.length; i++) {
            if (i === 0 && this.type3Config.rightList[i].scoreList) {
              ...
              data.scoreGiftConfig.fiveTypes = this.type3Config.rightList[i].scoreList[0];
            }
          }
    ...
    productSelect(item) {
      ...
      if (!isRepeat) {
        ...
        // 多类型才会所有型号礼品都一样
        if (this.type1Config.type === 3) {
          const curVirtualGiftListPar = this.type1Config.rightList.find(v => v.virtualGiftList);
          if (curVirtualGiftListPar) {
            this.$set(product, 'virtualGiftList', curVirtualGiftListPar.virtualGiftList);
            this.disableGenerate = false;
          }
        }
    ...
    deleteGift(item, index) {
      ...
      this.save({ temporary: true, getCostAmount: true });
    ...
    configGift(type, index, productCode, scoreList, giftList, modelNo, num) {
      ...
      /**
         ...
         * 1:单品单类型,尊享卡单品多类型
         * 2：单品全部
         * 3:套购独立配置、2345类或件数以上，5件以上按3件处理
         * 4:套购独立配置按满赠、产业内所有产品
         * 5:套购独立配置、按特定型号组合
         * 6：套购独立配置按满赠、产业内特定产品
         * @param scoreList 当前积分礼品列表
         * @param scoreList 当前实物礼品列表
         */
      if (type === 5) {
      ...
      // 虚拟卡也用giftList传值，应该再设置virtualGiftList参数，后期改造传参方式
      const listName = this.form.giftType === 5 ? 'virtualGiftList' : 'giftList';
      if (giftList && giftList.length > 0) {
        this.checked1 = 2;// 礼品
        this[listName] = giftList;
        this.intorpro = true;
      } else {
        this[listName] = this.form.giftType === 5
          ? [
            {
              cardType: ''// 尊享卡code
            }
      ...
      if (type === 1) {
        ...
        if (this.form.giftType === 5) {
          // todo 待修改 checked1名字不严谨、变量更改地方待确认
          this.checked1 = 2;// 送礼品
          this.getHonorEnjoyCards().then(() => {
            // 设置已选择的礼品
            const { cardType } = this.virtualGiftList[0];
            if (cardType) {
              const curCard = this.honorEnjoyCardList.find(v => v.cardType === cardType);
              this.$set(curCard, 'isActive', true);
            }
          });
        }
        this.saveGiftConfig = () => {
          /* 单型号配置保存 */
          // checked1 1：积分 2：礼品
          // rightType 1:固定积分 2：产品价格比例
          if (this.checked1 === 1) {
            if (
              (this.scoreList[0].rightType === 1 && this.scoreList[0].num && this.scoreList[0].score)
                || (this.scoreList[0].rightType === 2 && this.scoreList[0].priceScale && this.scoreList[0].salesAmount)
            ) {
              this[`type${type}Config`].rightList[index].scoreList = this.gUtils.simpleDeepCopy(this.scoreList);
              this[`type${type}Config`].rightList[index].giftList = [];
            } else {
              this.$message({
                type: 'warning',
                message: '请配置积分！'
            ...
            if (this.form.giftType === 5) {
              // checked1也为2，当做礼品
              // 尊享卡 单类型、多类型也用type1Config
              if (!this.virtualGiftList[0].cardType) {
                this.$message({
                  type: 'warning',
                  message: '请选择礼品！'
                });
              ...
              if (this.type1Config.type === 1) {
                this.$set(this.type2Config.rightList[0], 'virtualGiftList', this.gUtils.simpleDeepCopy(this.virtualGiftList));
              } else if (this.type1Config.type === 2) {
                this.type1Config.rightList[index].virtualGiftList = this.gUtils.simpleDeepCopy(this.virtualGiftList);
              } else {
                // 应后台要求，多类型虽然礼品一致，但是virtualGiftList在每个产品对象里都有
                this[`type${type}Config`].rightList.forEach((v) => {
                  v.virtualGiftList = this.gUtils.simpleDeepCopy(this.virtualGiftList);
                });
              }
            } else {
              if (!this.giftList[0].giftId) {
                this.$message({
                  type: 'warning',
                  message: '请选择礼品！'
                });
              ...
              this[`type${type}Config`].rightList[index].giftList = this.gUtils.simpleDeepCopy(this.giftList);
              this[`type${type}Config`].rightList[index].scoreList = [];
            }
          }
          this.save({
            temporary: true,
        ...
        this.saveGiftConfig = () => {
          if (
            ...
            if (this.scoreList[0].rightType === 1) {
              if (parseInt(_this.scoreList[0].num) === 0) {
                this.$message({
                  type: 'warning',
                  message: '礼品总数不能为0！'
              ...
              this[`type${type}Config`].rightList[index].scoreList = this.gUtils.simpleDeepCopy(this.scoreList);
            } else if (this.scoreList[0].rightType === 2) {
              if (parseInt(_this.scoreList[0].priceScale) === 0) {
                this.$message({
                  type: 'warning',
                  message: '产品价格比例不能为0！'
                ...
                return false;
              } if (parseInt(this.scoreList[0].salesAmount) === 0) {
                this.$message({
                  type: 'warning',
                  message: '产出销售额不能为0！'
                });
              ...
              this[`type${type}Config`].rightList[index].scoreList = JSON.parse(JSON.stringify(_this.scoreList));
            }
          } else {
            if (_this.scoreList[0].rightType == 1) {
              if (_this.scoreList[0].num == '') {
    ...
    giveHonourEnjoyCard(curCard) {
      /* 送尊享卡 */
      const activeCard = this.honorEnjoyCardList.find(v => v.isActive);
      activeCard && (this.$set(activeCard, 'isActive', false));
      this.$set(curCard, 'isActive', true);
      // 当前选择的尊享卡
      this.virtualGiftList = [curCard];
    },
    async getHonorEnjoyCards() {
      /* 获取尊享卡列表 */
      const data = {
        productType: this.productCategoryCode
      };
      this.honorEnjoyCardList = await this.requestRights.getHonorEnjoyCards(data) || [];
    },
    openImportExcelDialog() {
      /* 打开导入excel对话框 */
      this.choosedExcelName = '';
      this.isImportExcelDialog = true;
    },
    resolveExcel({ currentTarget: { files } }) {
      /* 解析excel */
      const file = files[0];
      const types = file.name.split('.')[1];
      this.choosedExcelName = file.name;
      ...
      this.file2Xce(file).then((tabJson) => {
        if (tabJson && tabJson.length) {
          /* 暂存已选的产品列表 */
          this.choosedExcel = tabJson[0].sheet.map(v => ({
            mmt_material_code: v['编码'],
            mmt_material_descrition: v['型号'],
            pro_band: v['品牌'],
            mmt_department: v['产业名称'],
            bpp_price: v['价格']
          }));
        }
      });
    },
    pushDataToSelectProduct() {
      /* 存入已选的产品列表 */
      this.choosedExcel.forEach((v) => {
        this.productSelect(v);
      });
      this.isImportExcelDialog = false;
    },
    file2Xce(file) {
