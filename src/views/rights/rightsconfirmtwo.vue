    ...
    getOrderRightsById() {
      ...
      this.requestRights.getOrderRightsById(params).then((data) => {
        if (data) {
          ...
          data.scoreGiftConfig.type = parseInt(data.scoreGiftConfig.type);
          _this.form = data;
          if (_this.form.rightSmallType == 2) {
            if (_this.form.scoreGiftConfig != null && _this.form.scoreGiftConfig.type == 3) {
              _this.modelNo = '';
              _this.cpname = false;
              for (let c = 0; c < _this.form.scoreGiftConfig.modelNoDetail.length; c++) {
                _this.form.scoreGiftConfig.modelNoDetail[c].modelNo = _this.form.scoreGiftConfig.modelNoDetail[
                  c
                ].modelNoList.join(',');
              }
            }
