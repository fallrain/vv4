  ...
  <div class="vip-content">
    <div class="vip-head-tool">
      <p class="template-brand">
        <i></i>标签名称<a href="javascript:;" class="color-link fr" @click="goback"
          ><i class="iconfont icon-chevron-copy-copy"></i>返回</a
        >
      </p>
      <p style="margin-bottom: 20px">{{ tagName }}</p>
      <p class="template-brand fl"><i></i>标签人员</p>
      <span style="line-height: 36px; margin-left: 15px;">人数：{{ msgListData.entityCount }}</span>
      <el-input
        placeholder="请输入手机号"
        icon="search"
        v-model="mobile"
  ...
  methods: {
    ...
    listSearch() {
      ...
      const data = {
        ...
        tagId: self.$route.query.id,
        type: 1,
        mobile: self.mobile
      };

      this.msgListLoading = true;
      this.requestVip.tagMember(data).then((data) => {
